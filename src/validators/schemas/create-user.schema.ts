...
export const createUserSchema = object({
  body: object({
    ...
    email: string().required('A user must have an email').email('A user must have a valid email'),
    password: string().required('A user must have a password').min(8, 'Password must be atleast 8 characters long'),
